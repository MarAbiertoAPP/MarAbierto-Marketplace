...
export default function Card ({ title, image, price, id, collectionName }) {
  ...
  return (
    <motion.div
    className='w-72 mx-6 my-4 text-white border rounded-lg border-[#5c1c5c6b] p-4 relative hover:shadow-[0_10px_50px_1px_rgba(25,25,205,0.3)]  hover:border-[rgba(25,25,205,0.3)]'
    initial={{ opacity: 0 }}
    ...
    >
        ...
        <div>
          <p className='flex items-center py-3 pb-16 text-sm'>{`Price: ${price}`} <FaEthereum /></p>
        </div>
        <button className='w-4/6 flex justify-center items-center bg-purple-900/80 hover:bg-purple-700 py-2 rounded-full absolute bottom-4 left-0 right-0 m-auto' onClick={(e) => isAuthenticated ? handleBuy(e) : loginWithRedirect()}><FiShoppingCart className='mr-2' />Add To Cart</button>
    </motion.div>
  )
}
