...
export default function Create () {
  ...
  const hanldeInputDescription = (input) => {
  ...
  const enviarNft = async () => {
    ...
    axios.post('https://api.cloudinary.com/v1_1/julian-soto/image/upload', formData).then(response => {
      ...
      .then(nftcreate.length !== 0 && dispatch(createNFT(nftcreate)))
  }
  return (
        ...
        <div className='w-full flex'>
          ...
          <div className='basis-6/12 mt-12 space-y-4 py-8'>
            ...
            <div className='flex flex-col'>
              ...
              <input onChange={(e) => handleInputnameChange(e.target.value)} className='mr-20 pl-2 bg-transparent border-white border rounded-lg text-neutral-300'></input>
            </div>

            <div className='flex flex-col border-xl'>
              <h1 className='text-xl text-purple-700'>Description</h1>
              <textarea onChange={(e) => hanldeInputDescription(e.target.value)} className='h-28 mr-20 pl-2 bg-transparent border-white border rounded-lg  text-neutral-300'></textarea>
            </div>
            ...
            { status === 'NFT' &&
              ...
              <input onChange={(e) => handlePriceChange(e.target.value)} className='mr-20 pl-2 bg-transparent border-white border rounded-lg text-neutral-300'></input>
            </div>
            }
 ...
 */}
                ...
                <div>
                  ...
                  <button disabled={typeof imageSelected === 'string'} onClick={() => { enviarNft() }} className={'bg-transparent enabled:hover:bg-blue-500 disabled:cursor-not-allowed  text-blue-700 font-semibold enabled:hover:text-white py-2 px-4 border border-blue-500 enabled:hover:border-transparent rounded'}>enviar</button>
                </div>
